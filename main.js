...
const flowersAsHTML = () => {
    ...
    */
   /*
        Create large flower string.
        String contains an <article> for each flower.
        For each flower, each color contains a <section>.
        Fer each flower, each USDA zone contains a <div>.
    */

    for(const flower of flowers){
        
        const colorString = flower.colors.join("</section>\n<section>")
        const zoneString = flower.usdaZones.join("</div>\n<div>")

        flowersHTMLString += `<article>\n<h1>${flower.type}</h1>`
        flowersHTMLString +=`<h2>Colors</h2>\n`
        flowersHTMLString +=`<section>${colorString}</section>\n\n`
        flowersHTMLString +=`<h2>USDA Zones</h2>\n`
        flowersHTMLString +=`<div>${zoneString}</div>\n</article>\n`
    }
